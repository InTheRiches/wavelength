    ...
    return (
        <div className="flex flex-col h-full w-full max-w-1/5">
            <div id="sidebar" style={{maxHeight: remainingHeight + "px"}}
                 className={`sidebar-taper pb-6 sticky top-20 overflow-y-auto z-10 px-4 mt-15 pr-4 text-neutral-900 dark:text-slate-50 hidden sm:flex flex-col`}>
...
function Topic({ topic, activeTopic }) {
    ...
    return (
        <div className={`${
             ...
             } flex items-center transition-all duration-200 hover:cursor-pointer hover:text-cyan-accent hover:dark:text-cyan-accent border-l-1 py-1 text-2xl`}
             onClick={() => {
    ...
    return (
        <div key={getNextKey()} className={`flex flex-col`}>
            <div key={getNextKey()}
                ...
                <a className={`text-2xl ml-6`}>{subcategory.title}</a>
