                ...
                <table className="myTable w-full">
                    ...
                    <tbody> {
                        workout.exercises.map((piece, index) =>
                            <tr>
                                <td style={{ paddingRight: "1rem" }} className={"py-2 h-[41px] " + (index !== workout.exercises.length-1 ? "border-b-1 border-neutral-700" : "")}>{piece.name}{piece.optional ? <div className={"inline w-fit mt-1 ml-2 rounded-xl border-1 border-sky-500 py-0.5 px-2 dark:bg-blue-500 dark:bg-opacity-5 text-sky-600 dark:text-sky-500 bg-blue-50 text-sm"}>OPTIONAL</div> : <></>}</td>
                                <td className={"py-2 h-[41px] " + (index !== workout.exercises.length-1 ? "border-b-1 border-neutral-700" : "")}>{piece.reps}</td>
                                <td className={"py-2 h-[41px] " + (index !== workout.exercises.length-1 ? "border-b-1 border-neutral-700" : "")}>{piece.sets}</td>
                            </tr>
                        )
                    }
                    </tbody>
