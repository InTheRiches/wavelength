  ...
  useEffect(() => {
    ...
    setProcessedJSX(jsx.map((item, i) => {
      ...
      return React.createElement(item.type,
          ...
          item.JSX ? item.JSX.map((innerItem, j) => React.createElement(innerItem.type, { key: j, onClick: innerItem.url ? () => handleClick(innerItem.url) : () => {}, className: innerItem.class }, innerItem.content)) : item.content)
    }));
    ...
    lines.forEach((line, i) => {
      ...
      if (line.startsWith('# ')) {
        ...
        jsx.push({
          ...
          "class": "min-[424px]:text-3xl text-2xl font-bold text-left flex items-center dark:text-slate-50 mt-12",
          "content": text
        });
        return;
      }
